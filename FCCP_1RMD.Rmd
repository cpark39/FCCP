...
FCCP.data2<-read.csv(file = "FCCP dataset_2.csv")
...
STRESS.agg.c <-scale(FCCP.data2$STRESS.agg, scale = FALSE)
SATISFACTION.c <-scale(FCCP.data2$SATISFACTION, scale = FALSE)
COMPETENCE.c <-scale(FCCP.data2$COMPETENCE, scale = FALSE)
EMOEXHAUST.c <-scale(FCCP.data2$EMOEXHAUST, scale = FALSE)
X3A_AGAIN.c <-scale(FCCP.data2$X3A_AGAIN, scale = FALSE)
X3B_12MO.c<-scale(FCCP.data2$X3B_12MO, scale = FALSE)
X3C_STR.c<-scale(FCCP.data2$X3C_STR, scale = FALSE)
FAM.agg.c<-scale(FCCP.data2$FAM.agg, scale = FALSE)
CONFLICT.agg.c<-scale(FCCP.data2$CONFLICT.agg, scale = FALSE)
CLOSENESS.agg.c<-scale(FCCP.data2$CLOSENESS.agg, scale = FALSE)
CCNES.enc.agg.c<-scale(FCCP.data2$CCNES.enc.agg, scale = FALSE)
CCNES.neg.agg.c<-scale(FCCP.data2$CCNES.neg.agg, scale = FALSE)
CCNES.pos.agg.c<-scale(FCCP.data2$CCNES.pos.agg, scale = FALSE)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
#Relationship models
Var.model1 <-lm(FAM.agg.c~SATISFACTION.c+COMPETENCE.c+EMOEXHAUST.c+X3A_AGAIN.c+X3B_12MO.c+X3C_STR.c+STRESS.agg.c, data=FCCP.data2)

Var.model2a <-lm(CONFLICT.agg.c~SATISFACTION.c+COMPETENCE.c+EMOEXHAUST.c+X3A_AGAIN.c+X3B_12MO.c+X3C_STR.c+STRESS.agg.c, data=FCCP.data2)

Var.model2b <-lm(CLOSENESS.agg.c~SATISFACTION.c+COMPETENCE.c+EMOEXHAUST.c+X3A_AGAIN.c+X3B_12MO.c+X3C_STR.c+STRESS.agg.c, data=FCCP.data2)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
#CCNES models
Var.model3a <-lm(CCNES.enc.agg.c~SATISFACTION.c+COMPETENCE.c+EMOEXHAUST.c+X3A_AGAIN.c+X3B_12MO.c+X3C_STR.c+STRESS.agg.c, data=FCCP.data2)

Var.model3b <-lm(CCNES.neg.agg.c~SATISFACTION.c+COMPETENCE.c+EMOEXHAUST.c+X3A_AGAIN.c+X3B_12MO.c+X3C_STR.c+STRESS.agg.c, data=FCCP.data2)

Var.model3c <-lm(CCNES.pos.agg.c~SATISFACTION.c+COMPETENCE.c+EMOEXHAUST.c+X3A_AGAIN.c+X3B_12MO.c+X3C_STR.c+STRESS.agg.c, data=FCCP.data2)
...
Box = boxcox(FCCP.data2$CCNES.enc.agg ~ 1,    # Transform CCNES.enc.agg as a single vector
...
CCNES.enc.box = (FCCP.data2$CCNES.enc.agg ^ lambda - 1)/lambda   # Transform the original data

library(rcompanion)
plotNormalHistogram(CCNES.enc.box)

FCCP.data2$CCNES.enc.box<-CCNES.enc.box #add transformed data to dataframe
CCNES.enc.agg.cb<-scale(FCCP.data2$CCNES.enc.box, scale = FALSE) #scale new data

#re-run regression
Var.model3a2 <-lm(CCNES.enc.agg.cb~SATISFACTION.c+COMPETENCE.c+EMOEXHAUST.c+X3A_AGAIN.c+X3B_12MO.c+X3C_STR.c+STRESS.agg.c, data=FCCP.data2)
summary(Var.model3a2)

write.csv(FCCP.data2,'FCCP.dataset_2.1.csv')
```
...
FCCP.data3<-read.csv(file = "FCCP dataset_3.csv")
...
#Variables included in cluster analysis (PENG, PCMT, Stress)
ClustVars <-FCCP.data3[c(15,16,18:20,79)]
head(ClustVars)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
fviz_cluster(list(data = ClustVars, cluster = grp.di4)) 
...
Clust.grp<-cbind(grp.di4,ClustVars) #assigning to groups

FCCP.data.groups<-cbind(Clust.grp,FCCP.data3)

write.csv(FCCP.data.groups,'FCCP dataset_4.csv')
```

#Group comparisons of clustering variables
```{r, echo=FALSE, warning=FALSE, message=FALSE}
FCCP.data4<-read.csv(file = "FCCP dataset_4.csv")

Clust.aov <- aov(grp.di4~X3A_AGAIN+X3B_12MO+SATISFACTION+COMPETENCE+EMOEXHAUST+STRESS.agg,data=FCCP.data4)
summary(Clust.aov)
#sig differences between groups

#group descriptives for clustering variables
data(ClustVars)
describeBy(ClustVars,FCCP.data4$grp.di4) 
```

#Group comparisons of all variables
```{r, echo=FALSE, warning=FALSE, message=FALSE}
#group descriptives for clustering variables
data(FCCP.data4)
describeBy(FCCP.data4,FCCP.data4$grp.di4) 
```
